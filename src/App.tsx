...
import { TranslateCode } from "./classess/code";
...
function App({
	...
	data: loadedAssets;
	name: string;
	socket: connectionResultType;
}) {
	// game usage
	const [actions, SetAction] = useState<ActionList | undefined>(undefined);
	const [playerList, SetPList] = useState<
		Array<{ name: string; balance: number; balanceTurn: number }>
	>([]);
					...
					{actions !== undefined ? (
						<>
							{actions.slider != undefined ? (
								<Slider
									defualtValue={
										actions.slider.defaultValue ?? 50
									}
							...
							<div
								className="actions"
								data-has={actions.slider != undefined}
							>
								{Object.entries(actions.functions).map(
									([value, func]) => (
										<button
											id={value}
											disabled={
												/*value === "Raise"*/ typeof func ===
													"object" && func.disabled()
											}
											onClick={() => {
												if (
		...
		<AsyncTask<connectionResultType>
			...
			resolve={(sock) => (
				<AssetLoader
					assets={urls}
					resolve={(data) => (
						<App data={data} name={args.name} socket={sock} />
					)}
				/>
			)}
		/>
